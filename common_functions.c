...
void print_value(const char *msg, microsf_s value) {
  printf("\r%s%s: %s", GREEN, msg, CYAN);
  print_binary_digits_precision(value, MICROSF_TOTAL_BITS);
  printf(" (0x%03x)%s%c", value & ((1 << MICROSF_TOTAL_BITS)-1), RST, IS_INTERACTIVE?'\n':'\0');
}
...
static void print_binary_digits_precision(unsigned int val, int precision) {
  if(precision == 0) {
    ...
    printf("%d", val&0x1);
    if(precision == MICROSF_SIGN_BITS) printf(" ");
    else if(precision == MICROSF_SIGN_BITS + MICROSF_EXP_BITS) printf(" ");
  }
} 
